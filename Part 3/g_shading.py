...
def g_shading(img, vertices, vcolors,option,bcoords, campos, ka, kd, ks, n, lpos, lint, lamb,vertsn,texture_map,uv_triangle):
    ...
    for y in range(ymin, ymax,1):#mexri y max => h oriakes times den xrwmatizontai
        ...
        for i in range(1, 2, 2):
            for x in range((x_values[0]),x_values[1]): 
                if option=="phong":
                    normal=pixel_color(y, allEdges, active_edges, vertices, vertsn, x)
                    color = pixel_color(y, allEdges, active_edges, vertices, vcolors, x)
                    final_color = light(bcoords,normal,color,campos,ka,kd,ks,n,lpos,lint,lamb)
                    img[y][x] = final_color 
                elif option=="texture map": 
                    normal=pixel_color(y, allEdges, active_edges, vertices, vertsn, x) 
                    uv=pixel_color(y, allEdges, active_edges, vertices, uv_triangle, x) 
